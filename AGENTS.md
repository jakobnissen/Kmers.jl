...
* This package relies heavily on zero-cost abstractions, which must inline to be zero-cost. The sheer number of helper functions sometimes cause the compiler to not inline, hence the many `@inline` annotations. When creating new functionality that might be performance critical, inspect its generated assembly code and determine if `@inline` is needed.

## Development guidelines
* Format with `runic -i .`. If Runic cannot be run, alert the user instead of finding workarounds for how to run Runic.

* This package depends heavily on BioSequences.jl internals. When in doubt about encoding, check BioSequences documentation.

* Kmers are optimized for small k. For k > ~100, LongSequence may be more appropriate.

* This package may assume a little-endian CPU, and refuse to compile on big-endian CPUs.
